...
def worker(q: "queue.Queue[tuple[int, dict]]"):
    while True:
        ...
        try:
            ...
            t1 = time.time()
            print(
                f"Published {i+1} topic={payload['topic']}, "
                f"http_latency={(t1 - t0)*1000:.2f} ms"
            )
...
def run_load(rate_msgs_per_sec: float, duration_sec: int):
    ...
    print(
        f"Sending {num_msgs} messages "
        f"round-robin topics={TOPICS} "
        f"at {rate_msgs_per_sec} msg/s for {duration_sec} seconds"
    )
    ...
    for i in range(num_msgs):
        ...
        # Chọn topic luân phiên: sports, weather, sports, weather, ...
